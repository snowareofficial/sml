...
**⚠️ 实现状态**：这个写法 **目前只有 JS 实现**（`js/sml.mjs` 里 `feats.has("typed-block") && feats.has("contract")`）。
Rust / C / C++ / Lua 里**没有这个特性名** —— 后果分两种，都很值得记住：

| 情形 | 实际发生 |
|---|---|
| 未实现 + 你又写了 `@feature enable typed-block` | **`E-FEATURE-003`（未知特性名）** ⇒ 文档在那端**直接不可用** |
| 未实现 + 你没写这条 `@feature` | 该写法**退化成普通裸块**（`__type: 契约名` + `__name: 块名` 两个元数据键） ⇒ **契约既不校验也不填默认值，还不报错**（静默差异） |

所以：**跨实现共享的文档不要用它**，写 `@is 契约名` 才可移植（`showcase_contract.sml` 就是这么改的）。
...
**潜力兑现的前提**：它得先在 **Rust（参考实现）** 上落地。在此之前，把它当**方言能力**看，
而不是通用语法 —— 这也是为什么本节开头要写"目前只有 JS 实现"。

...
### 词法层面的实现差异（2026-09-19 补测，判据同样是真机行为）

同一轮把**词法单元**也扫了一遍（20 个用例 × 5 端；判据是"用例里的键 `x` 是否幸存"，
这样能抓住"解析成功但键没了"这类**静默数据损坏** —— 只看"能不能解析"是抓不到的）：

| 用例族 | 结果 |
|---|---|
| 开头 **BOM**（U+FEFF） | ⚠️ **五端原先把 BOM 吃进第一个键名**（Rust 输出 `"\uFEFFx": 1`、JS 输出 `{"\uFEFFx":1}`）⇒ 本轮**已五端统一跳过**（见下） |
| 块注释 `/* */`、`_* *_`、跨行、**内含 `/*` 提及**、末尾无换行 | ✅ 五端一致 |
| 字符串：单行 / 多行 / 转义 / 内含 `#` `//` `/*` | ✅ 五端一致 |
| **三引号多行字符串** | ❌ **仅 Lua**：键被丢掉（未修，已登记） |
| CRLF 行尾、三种行注释、`@version` 行尾注释 | ✅ 五端一致 |
| `@feature … # 行尾注释` | ❌ **Lua 吞掉下一行**（未修，已登记）；C 报 `E-PARSE-005`（C 无 `@feature`，见上表） |

**BOM 的修法（五端各一处，全部按"文件级规范化"处理）**：Rust 在
`sml-parse::scan::strip_version`（**所有文本入口的唯一漏斗**）+ `sml-include::expand_file_tokens`（子文件）；
JS 在 `parse()`；C 在 `sml_parse` + `resolve_includes` 的子文件读取；C++ 在 `Parser::parse` + include 的子文件读取；
Lua 在 `Sml.load` + include 的子文件读取。每端都加了**断言"键名仍是 `x`"**的测试 ——
只断言"能解析"是抓不到它的（不去 BOM 也"解析成功"）。
⚠️ 顺带一提：仓库里 `smltools` 的 **YAML / XML 导入器早就各自去过 BOM**（`yaml.rs:93`、`xml.rs:63`），
只有 native SML 这条路径漏了 —— 属"同一件事两套待遇"，本轮补齐。

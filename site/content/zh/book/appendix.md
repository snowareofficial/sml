...
| 以前"能解析"的文档现在报错 | 它属于此前「静默给错树」的一类输入（未闭合字符串、数组里多余的 `}`、顶层标量等） | 按错误码修：多数是漏写引号或多写一个 `}` |

> 报错信息里的 `E-XXX-NNN` 是**稳定错误码**：同一类错误**五端同码**（Rust / C / C++ / JS / Lua），
> 用码到[错误码总表](/errors/)里检索，比抄一段文案去搜更准 —— 文案允许各端不同，码不允许。
...
本书随 `swsml 0.4.0` 发布。源码与更新：[/book/](/book/)。反馈请到 SML 仓库 issue 区。

...
- 路径**写法**非法（没加引号 / 引号没闭合 / 末尾多了字符）→ `E-INCLUDE-012`。
  以前这类错会被归成 `E-INCLUDE-001`（文件缺失），于是你去找一个"不存在"的文件，
  而真正的问题只是少写一对引号。
  ⚠️ **2026-09-19 补充**：这条**目前只有 Lua 在抛**。Rust 侧（含 `smltools`）未加引号的路径
  按"路径"解析 ⇒ 文件不存在时落到 `E-INCLUDE-001`；`smltools` 原有一份自己的"必须加引号"校验，
  已随「include 展开统一到库」一并删除（那份实现还会把 `include "a", "b" as sec` 的后半行
  **静默丢掉**）。要不要把"未加引号即非法"升为**语言级**规则**尚未定**，见 `TODO.md`。

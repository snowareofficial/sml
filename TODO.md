...
| **W16** ⏳ **进行中（用户裁决「全做」，逐端落地）** | 「静默清单」逐条判定并回填码表 | `errors/README.md` 的静默清单、各实现、`errors/codes.sml` | 清单里每条判定为「改成报错（给码）」或「写进规范、明确允许静默」，判定结果回填码表的 `status` 与规范文档。**先出判定表再动实现**（判定表 = `errors/silence-decisions.md`，含四端实测矩阵；**用户已裁决「全做」**）。**已落地**：Rust 顶层标量 `a8a37df`、JS 首批 `5bd0b64`、**C 批（2026-09-18）**（LEX 五条 + `E-PARSE-002/003/005/008` + `E-INCLUDE-006`；顺带对齐 `@contract X strict` 与片段显式参数；判别实验 HEAD 版 **20 条红**；全仓扫描 7 条 OK→FAIL **逐条查过根因、无一是误伤**）、**JS 余额 4 条（2026-09-18）**（未注册指令 `E-PARSE-005` + 片段显式参数 `E-PARSE-020`、未定义片段引用 `E-INCLUDE-006`、**特性门控** `contract`/`fragment` ⇒ `E-FEATURE-001` 与 `env` ⇒ `E-FEATURE-002`（安全修复：此前当沙箱关掉**实际没关**）、模式预算 `E-LIMIT-002`（JS 落在**编译期**，原生 RegExp 插不进计数器）；判别实验 HEAD 版 **13 条红**；同批修掉「四份副本漏同步」并新增闸门 `tools/check_js_copies.py`）。**未落地**：Lua 6 条 → C++ 6 条（先实测现状）→ Rust 侧 6 条 → **统一收口**（`impls` 全表回填 / README 静默清单终稿 / TODO 收尾）。进度与逐条记录见 `errors/silence-decisions.md` §4 | W11 的清单 | ✋ |

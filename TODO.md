...
| **W20** ⏳ **第一阶段（契约）已完成 2026-09-18；第二阶段（include）进行中** —— | ⚠️ **Lua 实现缺契约与 include 支持**（既有，非 W10 引入）：Lua 的 `parse_block` 对 `@contract` / `@is` / include **全无处理** —— `@is Service` 被当成片段定义、把紧随的 `name` 当"类型/名字"参数吃掉；`include "x.sml"` 被当成裸块键，把后面到第一个 `{` 的内容**全吞进片段体**。于是 `examples/app.sml`、`SML_政务数据密级标注规范_报送稿.sml` 这类文档**"能解析"但树是错的**（实测：全仓 41 个 `.sml` 里，改动前 28 OK，其中 2 个是"错树"）。W10 给键位置加码后它们改为明确报 `E-PARSE-006` —— **变响亮了，但能力缺口仍在** | `lua/lib/sml.soup` | 先决定再动手：① 补契约（`@contract` / `@is` 校验 + 默认值）与 include；② 或明确把 Lua 声明为**子集实现**并写进 README 能力矩阵，同时给"遇到不支持的指令"一个**更准确的码**（现在报的 `E-PARSE-006` 只是局部症状，不是根因）。**这是产品决定，不是补码** —— 先问用户 | W10 的键位置检查（已完成，正是它把问题暴露出来） | ✋ |
| **W21** | W10 尾巴（`smltools` 带码）落地时**登记下来的三处"没有家"的东西**，都不许自行加码/扩范围，故留待裁决：① **候选新码 `E-INCLUDE-012`**（include 路径写法非法，如未加引号）—— 现在暂归 `E-INCLUDE-001`，属**已知错码**（用户拿 `E-INCLUDE-001` 去查会看到"文件缺失或读取失败"，被误导）；② **候选新码 `E-CLI-008`**（通用命令行用法错误：clap 的未知参数/缺取值/非法枚举）—— 现在**完全无码**，clap 直印 + exit 2；③ `rust/smltools/src/toml.rs` 的 `descend` 里有个 `unreachable!()` —— **panic 是不可接受的对用户失败方式**，改成 `Err` 要动 `&mut BTreeMap` 借用结构。另有一处**根因**登记：`rust/src/emit/*` 仍返回**无码 `String``**，smltools 只好靠**文案前缀**把 `E-LIMIT-004/005/006`/`E-EXT-006` 映射出来（**⚠️ 此处我原先写错了**：那两条单测喂的是**自己的字面量**，钉的是**映射逻辑**、不是上游措辞 —— 所以上游一改文案，码会**静默**退化成 E-CLI-007（仍是合法码、只是粒度变粗），**没有任何告警**。w10-tail 主动更正了它自己报告里的这句过头话，我采纳。**加固用例（喂真实上游错误）本轮没做**：agent 在动笔前被强制收走（仓库未留半截改动，已核实工作区干净），而我**不打算 rush 一条"看起来像保护、实则仍在喂自己字面量"的测试** —— 那正是本轮花力气揭穿的失效模式。做法已由它给出（直接调 `sml::emit::to_custom` 造真实报错：100001 元素数组 + `{items:…}` 规则 → E-LIMIT-006；129 层嵌套 + 兜底规则 → E-LIMIT-004；005 需先实测），**关键是加完必须先用"临时改上游文案 → 用例变红"证明它真的会红**；彻底修法是让 emit 后端也返回 `SmlError`，跨 crate） | `errors/codes.sml`（加码时）、`rust/smltools/src/{main,toml}.rs`、`rust/src/emit/*` | ①/② 加不加码由用户拍板（加码=改对外契约）；③ 改 `descend` 的返回类型；④ emit 后端返回 `SmlError` 后删掉前缀映射与它的单测 | W10 尾巴（已完成） | ✋ |

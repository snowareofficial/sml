# SML Rust 实现 — 安全与正确性审计报告（完整台账：问题 + 现状）

> **怎么读这份文件**：§0-* 与 §一 是**最新的**（含各轮复测结果与修复确认），
> §二 及之后的章节是**审计当时的原文**，用来留住证据链（当时怎么观察到的、
> 影响面怎么判断的）。也就是说：**§二 的标题「待修复」是当时的结论，不是当前状态** ——
> 那里的 P0-1 ~ P2-1 现已全部处置，逐条现状见 §0 的速览表与 §一 的确认表。
...
### 新发现（**已于 2026-09-18 修复**）

**① `MiniRegex` 的量词 off-by-one**：前一字符已由 default 分支消耗一次，
而 `+` 分支又要求**至少再消耗一个**，故 `x+` 实际等价于 `xx*` ——
`^ab+c$` 匹配 `"abbc"` 但**不匹配 `"abc"`**。

复核后确认这**不是 `+` 一个量词的问题，而是三个全错**：原子总被强制消费一次，
量词只管「额外」次数，故 `x*` ≡ `xx*`、`x?` ≡ `xx?`。另外量词只认「前一个字符」，
所以 `[0-9]+` 这类「字符类 + 量词」整体失效。

**② 顺带翻出的第二个缺陷（本报告此前未记录）**：`^` 与 `$` 同时出现时，
`regex_matches` 只判「能匹配」而**没判「匹配到结尾」**——
`^conf\.sml$` 会匹配 `conf.sml.bak`（退化成前缀匹配）。这对「按文件名做 include 过滤」
是危险的**松判**：本不该被包含的文件会被包含进来。

**修法**（`sml-regex/src/lib.rs`）：把「匹配一个原子」从「重复几次」里拆出来 ——
新增 `Atom`（字面 / `.` / 字符类）与 `parse_atom`，量词与它作用的原子在同一处处理
（`?` 零或一、`*` 零或多、`+` 一或多，均贪婪 + 回溯）；`^` 分支补齐 `anchored_end` 校验。
顺手让独立量词（`^*a$`）、未闭合字符类、结尾悬空转义一律「不匹配」而不 panic。

**验收**：`sml-regex` 新增 9 个单测；`tests/security.rs::regex_still_matches_correctly`
由「按旧行为锁定」改为正确语义（并补 `^conf\.sml$` 不匹配 `conf.sml.bak` 的用例）；
ReDoS 预算测试仍通过；`cargo test --workspace` **467 通过 / 0 失败**。
版本按纪律升 `sml-regex` → `0.1.0-alpha.3`（改动匹配结果 = 破坏性语义变更）。
...
> ⚠️ **本节的「待修复」是审计当时（2026-08-31）的结论，不是当前状态。**
> 下列 P0-1 / P0-2 / P0-3 / P1-1 / P1-2 / P1-3 / P1-4 均已修复，P2-1 也已改为
> 「返回 NULL 并置错误」（见 `src/c_abi.rs` 的 `cstr`）。复测结果见
> [§0. 第二轮复测结果速览](#0-第二轮复测结果速览) 与 [§一、已修复确认](#一已修复确认回归验证通过无需再处理)。
> 保留本节原文是为了留住**证据链**，不是为了宣布它们还没修 —— 别据此重新开修。
...
> 同上：这是**审计当时**的排期建议，现已按它做完了（见 §一）。留作方法论参考，
> 不是待办清单。

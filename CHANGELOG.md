...
- 🔴 **VSCode 扩展：按「启用」会把用户其他文件的图标全弄没（提示语与行为不一致）**
  ...
  （仅 .sml）** —— 那个主题的兜底是「无图标」，于是用户一按「启用」，`.md` / `.json` / `.py`
  等**所有非 SML 文件的图标一起消失**（用户实测反馈）。**文案是对的、代码是错的**，属于最难自查的一类。
  修法：① 一律切到 **`sml-icons-seti`**（SML + Seti 兜底，其他文件沿用 Seti）；
  ② 用户已有**第三方**图标主题时**不硬换**，改为提示并把他送到「文件图标主题」选择器；
  ③ 对已经中招的人（当前正是 `sml-icons`）给一次性修复提示「换成 SML + Seti」；
  ④ 新增命令 **`SML: 文件图标主题`**（SML+Seti / 仅 .sml / 打开 VS Code 选择器）——
  图标主题是**全局设置**，用户未必知道"整个工作区图标没了"是本扩展干的，必须给一条能切回来的路。
  闸门：`_verify_activate.mjs` 新增 **3 条行为断言**（记录 `update` 调用，断言「启用」后写成
  `sml-icons-seti`、**绝不**写成 `sml-icons`、命令已注册）。⚠️ 顺手补上 mock 缺 `globalState`
  的**假绿灯**：此前进程在异步提示跑起来之前就 `exit` 了，所以那个洞一直没暴露（加了等待才现形）。

- **Zed 侧文档与实情对齐**（`editors/zed/README.md`）：补一张**两编辑器能力对照表**
  （Zed：高亮 / 括号 / 注释；VS Code：另有诊断、补全、悬浮（契约/块/字段）、跳转、
  特殊颜色、特别高亮、自检、`.sml` 图标），并写明**本轮 VS Code 侧的功能扩展不影响 Zed 目录**
  （grammar / `highlights.scm` / `extension.toml` 的 `version` 与 `[grammars.sml].rev` 都没动
  —— 改动的判据是"是否碰了语法"，不是"VS Code 侧发了版"）。

- **新增 `TASK-hy3-w19.md`（给"不需要判断力"的执行者：编辑期能力手工验收单）**：
  逐条给出**敲什么命令 / 点什么按钮 / 期望看到什么 / 不符怎么办** —— 三关闸门、包内 vs 工作区
  指纹核对、`SML: 自检` 面板逐行判读表、6 项手工验收（契约名/块名/字段两种位置悬浮、三种跳转、
  特殊颜色、特别高亮、图标主题回归点）、Zed 侧 `tree-sitter generate/parse`、两条历史坑的现场复核
  （管道吃退出码 / CRLF 注释剥离）、文档同步 5 处 + 搜索索引重生成、验收清单与证据表。

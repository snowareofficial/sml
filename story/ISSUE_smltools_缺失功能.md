...
## 一、CLI 标题推断：注释与实现不符（**已修**）

> ⚠️ **原判据有误，此处更正。** 初版报告写的是「`@feature base <name>` 被解析器拒绝，
> 而 CLI 的标题推断依赖它」—— 逐一核对源码后，真实情况是另一回事：
>
> - `infer_title()` 的旧注释声称「尝试从 SML 文本中提取 `@feature base <name>` 作为标题」，
>   但**实现里从来没有这段逻辑**；而 `@feature base` 只接受版本号（v1/v2/v3/v4），
>   本来就承载不了名字 —— 注释与实现各说各话。
> - 该函数会读顶层 `__name`，但两个调用点（`write_hugo` / `write_zola`）都传 `&Value::Null`，
>   该分支**从未被触发过**（死代码）；顶层也基本不会出现 `__name`
>   （它来自片段/块级标注的**字段值**，不是顶层键）。
> - **真实后果**：`--hugo` / `--zola` 的标题只能取到文件名，文档里的标题信息完全读不到。
>   `@feature base "书名"` 确实会解析失败，但那与标题推断无关，是另一条独立约定问题。

**已修（2026-09-18）**

- 标题推断改为 `--title` > 文档顶层 `title` 字段 > 输入文件名 stem > `"doc"`，
  并把**真实解析结果**传入（原先传 `&Value::Null`，文档内的标题永远读不到）
- 删除永不触发的 `__name` 分支，以及两处描述错误的注释
- 顺带修 `sanitize_filename` / `sanitize_section`：原先只认 ASCII，
  中文标题会被整体替换成下划线（`我的长篇小说` → `______`），
  文件名与章节目录名里的标题信息全丢；现保留 Unicode 字母数字
  （安全性不变：`/`、`\` 仍被替换，首尾 `.` 仍被 trim，`..` 逃不出目标目录）

**遗留（另一条约定问题，仍未定）**：`@feature base` 只认版本号，
若希望「在文档里声明书名」，需要单独定一个约定（如顶层 `title:` 字段或新的指令），
目前 CLI 已按顶层 `title:` 字段推断，可作为事实约定。

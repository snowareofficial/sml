...
5. 能力范围只到 **高亮 / 括号匹配 / 注释切换**。VSCode 侧 richer 的那些能力都来自那里的
   扩展代码（`editors/vscode/src/`），**不是 grammar 能做到的**；Zed 侧要同样的能力得另写
   LSP 接入，本骨架不包含。两边各自有什么（2026-09-19 对照）：

   | 能力 | Zed（本目录） | VS Code（`editors/vscode/`） |
   |---|---|---|
   | 语法高亮 / 括号匹配 / 注释切换 | ✅ | ✅（另有自定义高亮 `HL-cfg.sml`） |
   | 实时诊断（解析错误） | ❌ | ✅ |
   | 补全（指令 / 契约 / 类型 / 键 / 片段） | ❌ | ✅ |
   | 悬浮：契约声明 + **契约填充后的结构** / 块名路径 / **字段规格与当前值** | ❌ | ✅ |
   | 跳转：`@is X` → `@contract X`、`&x` → `@x`、数据键 → 契约字段 | ❌ | ✅ |
   | 特殊颜色（右键写入 `HL-cfg.sml`，按语法单元生效） | ❌ | ✅ |
   | 特别高亮（一个词点亮整个工作区） | ❌ | ✅ |
   | 自检（把「为什么没反应」摊开写进输出面板） | ❌ | ✅ |
   | `.sml` 专用文件图标 | ❌（Zed 的图标机制不同） | ✅（含「SML + Seti」合并主题） |

   > 想要上面这些，请在 VS Code 里装 VSIX（见仓库根 `README.md` 的「编辑器支持」）。
   > **Zed 装不了 `.vsix`** —— 两边的包格式不通用。

6. **本轮（2026-09-19）VS Code 侧的能力扩展不影响本目录**：grammar（`grammars/sml/grammar.js`）、
   `highlights.scm`、`extension.toml` 的 `version` / `[grammars.sml].rev` **都没动**
   —— 新增的悬浮/跳转/特殊颜色/图标主题全是 VS Code 扩展的代码行为，不是语法。
   改动它们才需要同步（见上面第 2 条与 `grammars/sml/grammar.js` 文件头的节点名契约）。

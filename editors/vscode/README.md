...
| **悬浮（块名）** | 光标停在块名上（`primary {`，或**块级类型标注** `Server primary {`）：显示**路径**（`database.primary`）、它应用的契约、以及**契约填充后的实际结构**；没有契约的块也给结构。⚠️ 块级类型标注写法需 `@feature enable typed-block`，且**目前仅 JS 实现**（Rust/C/C++/Lua 会报 `E-FEATURE-003`）—— 跨实现共享的文件请写 `@is 契约名` |

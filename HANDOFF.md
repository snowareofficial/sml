...
- **错误码（已全量）**：`errors/codes.sml` 是唯一事实来源 —— 它本身用 SML 写，所以
  ...
  给官网 `/errors` 页。**135 条码 / 14 个领域 / 分四层**（语言层、宿主绑定层、工具层、
...
> 另有两处历史残留的自相矛盾：**`rust/AUDIT_REPORT.md` 的 §二「待修复缺陷」清单仍未清** ——
> 它列的 P0-1（parse 数组路径缺深度检查）、P0-2（Value 递归 Drop）、P0-3（emit 递归 clone）、
> P1-1、P1-2 **在代码里都已修**，同一份文件上方的 §0-* 也标了「已修复」，属两处口径打架
> （本会话只更新了其中的 `sml-regex` 部分，没去动这处矛盾）；`rust/README.md` 的自相矛盾
> 本会话已修。

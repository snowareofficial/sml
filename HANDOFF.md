...
### 22.13 词法层面：BOM 五端统一跳过（+ 又踩了两脚"探针自身"）

**用户的两问**：「拓展对命名空间的支持？一个个全做」→ 已做（CLI 私有 include 预处理删掉、统一走库
`parse_file`，见 §22.11）→ 「还有被遗漏的语法吗？」→ 本轮扫的就是这个。

**方法**：**20 个用例 × 5 端**，判据是"**用例里的键 `x` 是否幸存**" —— 这是关键设计：
BOM 这类问题**解析照样成功**，只断言"能解析"完全抓不到，必须断言"键名对不对"
（"解析成功但键没了/改名了" = 静默数据损坏，比报错更坏）。探针 `%TEMP%\probe_syntax.py`。

**发现（真缺口）**：

1. **开头 BOM（U+FEFF）**：五端**原先都**把 BOM 吃进第一个键名（Rust `"\uFEFFx": 1`、
   JS `{"\uFEFFx":1}`）⇒ 记事本「另存为 UTF-8」存出来的文件，第一个键名被**静默**改掉。
   **本轮五端统一跳过**，并各加一条"断言键名仍是 `x`"的测试（Rust 那份还覆盖 **include 子文件**）。
   ⚠️ 同一个仓库里 `smltools` 的 **YAML/XML 导入器早就去过 BOM**（`yaml.rs:93`、`xml.rs:63`），
   native SML 漏了 —— **"同一件事两套待遇"是找 bug 的高产线索**。
2. **Lua 两处只登记未修**（用户定）：三引号多行字符串**丢键**；`@feature … # 行尾注释`**吞掉下一行**。

**又踩的两脚（都在探针上，值得记）**：

1. **`smltools --from sml` 不加 `--to` 时默认输出 Markdown**（`- **x**: 1`）⇒ 我的判据找 `x:`，
   于是整个 `RustStdin` 列**假报"丢键"**；只影响探针，不影响实现。
2. **JS 判据只看顶层键** ⇒ `b { x: 1 }` 里嵌套的 `x` 被误判成"丢了"。
   两脚合起来一度让矩阵有 15 个 `DROP!` —— **不先校准就会拿着假矩阵去改代码**。

**还有一个真发现（C++ 用例写错）**：C++ 的 include 是 `@include "x"`，我写成 `include "x"`，
C++ 把它当**普通键**输出（`include: child.sml`）⇒ 差点又得出"子文件未展开"的假结论。

**验收装置**：`%TEMP%\bom_bytes.py`（五端 × 主文件/子文件，**字节级**查 `EF BB BF`；
BOM 在终端里看不见，肉眼会骗人）、`%TEMP%\cpp_bom.py`（自编 C++ runner；
**仓库的 `cpp/build_verify.py` 那条长命令被取消两次**，故改用 15 行 runner 自证）。

**给下一位**：改完 JS 别忘 `python tools/check_js_copies.py --fix` + 更新
`editors/vscode/src/extension.js` 的 `EXPECT_VENDOR` + `_prepublish.mjs`（**不加管道**）+ 重打 VSIX，
然后两份站点下载目录（`site/static/dl/` 与 **`site/public/dl/`** —— 后者是 Hugo 产物，容易漏）都要更新。
...
1. **实现面从来没人写**：`typed-block` **只在 JS 里实现**（`js/sml.mjs`：
   `feats.has("typed-block") && feats.has("contract")`）；**Rust / C / C++ / Lua 源码里没有这个特性名**
   （`rust/src` 只在 C-ABI 的特性名表登记了 bit 14 —— 名字被登记 ≠ 实现了）。
   而 `errors/codes.sml` 里有 **`E-FEATURE-003`（未知特性名）** ⇒ 两端后果：
   ① 未实现 + 写了 `@feature enable typed-block` ⇒ **文档在那端直接不可用**；
   ② 未实现 + 没写 ⇒ **退化成普通裸块**（`__type`/`__name` 元数据），契约**不校验、不填默认、不报错**
   —— 这是最危险的一种：看起来一样，语义不一样。
2. **`showcase_contract.sml` 是假示例**：它用了这个写法却**没开特性** ⇒ 契约**静默没生效**，
   而注释写着「loose 下允许；严格模式会报错」（严格模式压根没跑）。**实测证据**（本机 JS 探针）：
   不开特性 + 缺必填字段 ⇒ `ok=true` 不报错；开了特性 ⇒ `contract: Metrics — 契约字段缺失: latency`。
   已改成可移植写法（块内 `@is Metrics`），改后 `metrics` 块从
   `{latency, customCounter, __type, __name}`（裸块）变成 `{latency, customCounter}`（**契约真应用了**）。
   ⚠️ **不能**用"给它加一条 `@feature enable typed-block`"来修 —— 那会让 Rust/C/C++/Lua 报
   `E-FEATURE-003`，把跨实现共享的展示文件变成单端文件。
...
两者都是"文案与事实不一致"，和 §22.14 的图标主题事故是同一族问题。

...
codes: [
    ...
    { id: E-INCLUDE-012 domain: INCLUDE severity: E title: "include 路径写法非法"
      ...
      impls: [ rust lua smltools ] status: done
      note: "**2026-09-19 升为语言级规则**（用户裁决「好的」）：实现在 `sml-include::parse_include_line`，判据来自 `next_token_meta` 报告的「是否以引号开头 / 引号是否闭合」——未加引号、引号未闭合、通配裸写都报此码。**两处豁免**：① `import a.b`（点分模块名的裸词是**语法**的一部分）② `re:…`（受限正则模式首字符本来就是 r）。CLI（smltools）经库原样拿到此码。此前 Rust 侧把「未加引号」当普通路径解析 ⇒ `include nope.sml` 报 `E-INCLUDE-001`（文件不存在），用户被引去查一个**根本不该存在的文件**，而真正的问题是少了一对引号；「引号未闭合」更糟：静默把已读内容当完整路径。Lua 自 W20 第二阶段即如此（`include_line_path`：未加引号 / 引号未闭合 / 多余字符，`include` 与 `@include` 一致）。⚠️ **尚未做**：多余字符（闭合引号后面还跟着别的东西）；以及 JS / C / C++ 三端的对齐 —— JS 走裸词回退、C++ 的引号串与裸词同型（都接受未加引号）、C 只认 T_STR（未加引号的行**静默当普通内容**，连错都不报）" }

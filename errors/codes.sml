...
codes: [
    ...
    { id: E-INCLUDE-012 domain: INCLUDE severity: E title: "include 路径写法非法"
      ...
      impls: [ lua ] status: partial
      note: "⚠️ **2026-09-19 起 CLI（smltools）不再抛此码**：CLI 原先自带一份行级 include 展开（要求路径加引号，且把 `include \"a.sml\", \"b.sml\" as sec` 的后半行**静默丢弃**），现已删除、统一走库的 `sml::parse_file`；于是 `include nope.sml` 按库的语义走 ⇒ 报 `E-INCLUDE-001`（当路径解析后文件不存在）。若将来要把「未加引号即非法」定成**语言级**规则，必须在 `sml-include` 里实现（单一事实来源），那时再把 rust 加进 impls。**Lua 仍抛此码**（W20 第二阶段：未加引号 / 引号未闭合 / 多余字符，`include` 与 `@include` 一致）" }
